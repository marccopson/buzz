...
mod tests {
    ...
    async fn oversized_hint_is_capped_in_relay_error_message_string() {
        use crate::relay_admission::{reset_rate_limit_gate, tests::TEST_SERIAL, MAX_HINT_SECONDS};
        use std::io::{Read as _, Write as _};

        // relay_error_message arms the process-wide admission gate. Serialize
        // with the gate tests and clear both ends so this test cannot leave a
        // 300-second window racing the rest of the desktop suite.
