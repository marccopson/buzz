...
fn identifier_prefix_does_not_match_longer_id() {
    ...
    let buf = br#""identifier":"com.macsurfacing.workspace.dev""#;
    let id = b"com.macsurfacing.workspace";
...
fn identifier_prefix_does_not_match_worktree_slug() {
    ...
    let buf = br#""identifier":"com.macsurfacing.workspace.dev.my-branch""#;
    let id = b"com.macsurfacing.workspace.dev";
    assert!(!super::buffer_contains_identifier(buf, id));
}
...
fn identifier_match_with_null_boundary() {
    ...
    let mut buf = b"BUZZ_MANAGED_AGENT=com.macsurfacing.workspace.dev".to_vec();
    buf.push(0);
...
fn identifier_exact_match_at_end_of_buffer() {
    ...
    let buf = b"com.macsurfacing.workspace.dev";
    let id = b"com.macsurfacing.workspace.dev";
    assert!(super::buffer_contains_identifier(buf, id));
}
...
fn longer_id_matches_when_short_prefix_also_present() {
    ...
    let mut buf = b"com.macsurfacing.workspace".to_vec();
    buf.push(0);
    buf.extend_from_slice(br#""identifier":"com.macsurfacing.workspace.dev""#);
    let id = b"com.macsurfacing.workspace.dev";
    assert!(super::buffer_contains_identifier(&buf, id));
}
...
fn marker_entry_is_namespaced_by_instance_id() {
    ...
    assert_eq!(
        super::buzz_marker_entry("com.macsurfacing.workspace"),
        b"BUZZ_MANAGED_AGENT=com.macsurfacing.workspace".to_vec()
    );
    assert_ne!(
        super::buzz_marker_entry("com.macsurfacing.workspace"),
        super::buzz_marker_entry("com.macsurfacing.workspace.dev")
    );
}
