...
fn canonical_dev_data_dir_replaces_last_component() {
    ...
    assert_eq!(
        canonical,
        PathBuf::from("/Users/me/Library/Application Support/com.macsurfacing.workspace.dev")
    );
}
...
fn setup_sync_layout() -> (tempfile::TempDir, PathBuf, PathBuf) {
    ...
    let worktree = parent
        .path()
        .join("com.macsurfacing.workspace.dev.my-branch");
    let main_instance = parent.path().join("com.macsurfacing.workspace.dev.main");
...
fn seed_up_migrates_sibling_file_to_canonical_then_symlinks() {
    ...
    let sibling = canonical
        .parent()
        .unwrap()
        .join("com.macsurfacing.workspace.dev.main");
