...
export function effectiveBaseLineCount(baseLines, syncedBaseline) {
  if (syncedBaseline == null) {
    return baseLines;
  }
  return baseLines == null
    ? syncedBaseline
    : Math.max(baseLines, syncedBaseline);
}
...
export function resolveBaseRef(repoRoot, env = process.env) {
  ...
  try {
    const integrationRef = (() => {
      try {
        git(["rev-parse", "--verify", "--quiet", "origin/mac"], repoRoot);
        return "origin/mac";
      } catch {
        return "origin/main";
      }
    })();
    const mergeBase = git(["merge-base", integrationRef, "HEAD"], repoRoot).trim();
    const head = git(["rev-parse", "HEAD"], repoRoot).trim();
    return mergeBase === head ? "HEAD" : mergeBase;
  } catch (error) {
    throw new Error(
      "Could not resolve the file-size base from origin/mac or origin/main. Fetch the integration branch or set CHECK_FILE_SIZES_BASE to an explicit commit.",
      { cause: error },
    );
  }
}
...
export async function runFileSizeCheck({
  ...
  for (const change of changedProjectFiles({
    ...
    const repositoryBaseLines =
      baseContent == null ? null : countLines(baseContent);
    const baseLines = effectiveBaseLineCount(
      repositoryBaseLines,
      syncedBaselines.get(relativePath),
    );
