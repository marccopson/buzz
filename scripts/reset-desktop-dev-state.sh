...
# Production state (`com.macsurfacing.workspace`, `~/.buzz`, and `buzz-desktop`) is
...
remove_bundle_state() {
  ...
  [[ -d "$base" ]] || return 0
  shopt -s nullglob
  for prefix in com.macsurfacing.workspace.dev xyz.block.buzz.app.dev xyz.block.sprout.app.dev; do
