#!/usr/bin/env bash
# Uninstalls stale worktree-suffixed MAC Workspace debug builds from booted iOS
# simulators and connected Android devices/emulators. Production installs
# (com.macsurfacing.workspace, no suffix) are never touched:
...
ios_prefix="com.macsurfacing.workspace."
android_prefix="com.macsurfacing.workspace."
...
if [[ "$removed" == "0" ]]; then
    echo "no worktree-suffixed MAC Workspace installs found (production apps untouched)"
elif [[ "$dry_run" == "1" ]]; then
